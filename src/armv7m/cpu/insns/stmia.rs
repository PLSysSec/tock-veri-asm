...
impl Armv7m {
    ...
    #[flux_rs::sig(
        ...
        requires
            ...
            &&
            is_valid_write_addr(bv_add(get_gpr(rd, old_cpu), 0x4))
            &&
            is_valid_write_addr(bv_add(get_gpr(rd, old_cpu), 0x8))
            &&
            is_valid_write_addr(bv_add(get_gpr(rd, old_cpu), 0xc))
            &&
            is_valid_write_addr(bv_add(get_gpr(rd, old_cpu), 0x10))
            &&
            is_valid_write_addr(bv_add(get_gpr(rd, old_cpu), 0x14))
            &&
            is_valid_write_addr(bv_add(get_gpr(rd, old_cpu), 0x18))
            &&
            is_valid_write_addr(bv_add(get_gpr(rd, old_cpu), 0x1c))
