            ...
            map_set(
                map_set(
                    map_set(
                        map_set(
                            cpu.mem,
                            bv_sub(get_sp(cpu.sp, cpu.mode, cpu.control), 0x14),
                            get_gpr(r1, cpu)
                        ),
                        bv_sub(get_sp(cpu.sp, cpu.mode, cpu.control), 0x10),
                        get_gpr(r2, cpu)
                    ),
                    bv_sub(get_sp(cpu.sp, cpu.mode, cpu.control), 0xc),
                    get_gpr(r3, cpu)
                ),
                bv_sub(get_sp(cpu.sp, cpu.mode, cpu.control), 0x8),
                get_gpr(r4, cpu)
            ),
            bv_sub(get_sp(cpu.sp, cpu.mode, cpu.control), 0x4),
    ...
    #[flux_rs::sig(
        fn (
            ...
            requires
                ...
                &&
                is_valid_ram_addr(bv_sub(get_sp(old_cpu.sp, old_cpu.mode, old_cpu.control), 0x14))
            ensures self: Armv7m { new_cpu: new_cpu == Armv7m {
                    ...
                    sp: set_sp(
                        ...
                        old_cpu.mode,
                        old_cpu.control,
                        bv_sub(get_sp(old_cpu.sp, old_cpu.mode, old_cpu.control), 0x14)
                    ),
                    ..old_cpu
                }
            }
